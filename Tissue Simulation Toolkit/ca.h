...
public:
  ...
  */
  ...
  inline int SizeY() const {
	...
	double TissueElongation(void); //Calculates how much a tissue is elongated with help of inertia moment
	double Orderliness(void); //Calculates sites neighbouring sites of another type divided bythe total number of neighbours
	void WriteData(char* Iteration); //Outputs data for a certain time step
  void WriteCellCoordinates(int Time); //Outputs cell coordinates for a certain time step
  ...
  */
  ...
  void SetTypesLocation(void);
  void SetTypesLocationPercentage(void);
